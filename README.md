...
| MOSS Delay (MOSS-TTS-v1.5 / MOSS-TTSD) | tts | model-advertised multilingual text | 24 kHz | `f32`, `f16` | CPU; GPU offload implemented, not yet validated | Qwen3 backbone over a 32-channel delay pattern + RVQ codec, zero-shot cloning from a reference WAV; generation logic pinned by a conformance suite, checkpoint validation pending |
