...
| `TTS_CPP_COREML` | `OFF` | Apple-only. Compile the Core ML (Neural Engine) sidecar for the Audio8 codec synthesis stack: at load the engine looks for `<codec-decoder>.mlmodelc` next to the decoder GGUF (`audio8-codec-decoder-q8_0.gguf` → `audio8-codec-decoder.mlmodelc`) and runs synthesis on it when present, on ggml otherwise. Export the sidecar with `scripts/export-audio8-codec-coreml.py`; see the [Audio8 guide](audio8.md#core-ml-codec-sidecar). A non-Apple configure with this `ON` is a hard error |
