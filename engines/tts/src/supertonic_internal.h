...
// given shapes -- the SAME builders the runtime dispatches, reserved through
// ggml's size-only APIs; nothing is allocated and nothing runs.  Every one of
// these caches stays resident once its stage has run, so a projection SUMS
// them.  GPU (non-CPU-kernel) dispatch paths only: the CPU vector-estimator
// multi-cache path is not modelled (the fitter refuses it; see fit.h).
